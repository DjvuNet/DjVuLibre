...
provided by \Ref{IFFByteStream.h} to display an indented or json
formatted representation of the chunk structure of an ``EA IFF 85''
file. 

In case of indented format each line represent contains a chunk
ID followed by the chunk size. Additional information about 
the chunk is provided when program #djvuinfo.cpp# recognizes 
the chunk name and knows how to summarize the chunk data. 
Furthermore, page identifiers are printed between curly braces 
when #djvudump# recognizes a bundled multi page document. 
Lines are indented in order to reflect the hierarchical 
structure of the IFF files.

In case of json format the file is self documenting and can
be used directly for automatic consumption by any json syntax
aware software or human.
...
#endif
        "djvudump outputs DjVu and IFF85 files structure information\n\n" 
        "Usage:\t\tdjvudump [options] <iff_filenames>\n\n" 
        "Options:\n"
        "Output file:\t[ -o outputfile ]\n"
        "Output stream\t[ - ] stdout\n"
        "Input stream\t[ - ] stdin\n"
        "Output format:\t[ -j ] (use for json format)\n\n"
        "Example:\tdjvudump -j -o output.json ../../article.djvu\n\n"
    );
    exit(1);
}
